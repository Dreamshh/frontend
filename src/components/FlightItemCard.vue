    ...
    <v-row>
      ...
      <v-col cols="12">
        <v-card-text>
          ...
          <br>
          <v-btn small color="blue" class="mb-3 mr-3" @click='$router.push(`/flight/upd/${ flightItem.id }/`)'>Редактировать</v-btn>
          <br/>
          <v-btn small color="blue" class="mb-3 mr-3" @click="deleteElem(flightItem.id)">Удалить</v-btn>
          <br>
          <v-btn small color="blue" @click="$router.push(`/flight/${ flightItem.id }/ticket/`)">Оформить билет!</v-btn>
        </v-card-text>
      </v-col>
    </v-row>
  </v-card>
